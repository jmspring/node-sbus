    ...
    init: function(receivecb, callback) {
      ...
      function do_init_partitions(partitions, callback) {
        ...
        for(var i = 0; i < keys.length; i++) {
          ...
          debug('Instantiating partition ' + key + ' for ' + instance.eventhub.name + ', group ' + instance.group);
      ...
      if(!instance.runtime.initialized) {
        ...
        keys.forEach(function(key) {
          tasks.push(function(cb) {
            debug('Starting receipt for ' + key);
            // Partitions are in charge of updating their own offsets.
            instance.runtime.partitions[key].receive(callback);
          });
        });
