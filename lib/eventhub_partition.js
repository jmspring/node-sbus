  ...
  var instance = {
    ...
    update_partition_state: function(callback) {
      if(instance.storage.instance) {
        state = instance.eventhub.get_partition_state(instance.group, instance.id);
        if(state) {
          var s = {};
    ...
    subscribe: function(callback) {
      ...
      function do_subscribe() {
        if(("state" in partitionInfo) && ("x-opt-offset" in partitionInfo.state)) {
          filter = [ [ "symbol", "apache.org:selector-filter:string" ], [ "described", [ "symbol", "apache.org:selector-filter:string" ], ["string", "amqp.annotation.x-opt-offset > '" + partitionInfo.state["x-opt-offset"] + "'" ] ] ];
        }
